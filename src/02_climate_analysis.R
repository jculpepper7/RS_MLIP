...
ice_on_annual_rf <- randomForest(sen ~ ., data = mk_on_clim_annual, ntree = 500, localImp = TRUE)
plot(ice_on_annual_rf)
#plotmo(ice_on_annual_rf) # This shows relationships between top interactions but we get a bit more specific below.

ice_on_seasonal_rf <- randomForest(sen ~ ., data = mk_on_clim_seasonal, ntree = 500, localImp = TRUE)
plot(ice_on_seasonal_rf)

ice_on_monthly_rf <- randomForest(sen ~ ., data = mk_on_clim_monthly, ntree = 500, localImp = TRUE)
plot(ice_on_monthly_rf)

#Step 2: Get minimum depth distribution for variables
#Get the distribution of minimum depth for ice on

#Annual
min_depth_frame_on_annual <- min_depth_distribution(ice_on_annual_rf)

#Seasonal
min_depth_frame_on_seasonal <- min_depth_distribution(ice_on_seasonal_rf)

#Monthly
min_depth_frame_on_monthly <- min_depth_distribution(ice_on_monthly_rf)
#head(min_depth_frame_on_annual, n = 10)


#Plot the dist. of min. depth for ice on
#Annual
plot_min_depth_distribution(min_depth_frame_on_annual, min_no_of_trees = 30,  mean_sample = "relevant_trees")

#Seasonal
plot_min_depth_distribution(min_depth_frame_on_seasonal, min_no_of_trees = 30,  mean_sample = "top_trees")

#Monthly
plot_min_depth_distribution(min_depth_frame_on_monthly, min_no_of_trees = 30,  mean_sample = "relevant_trees")
...
#Step 1: Create Random Forest model
ice_off_annual_rf <- randomForest(sen ~ ., data = mk_off_clim_annual, ntree = 500, localImp = TRUE)
plot(ice_off_annual_rf)

ice_off_seasonal_rf <- randomForest(sen ~ ., data = mk_off_clim_seasonal, ntree = 500, localImp = TRUE)
plot(ice_on_seasonal_rf)

ice_off_monthly_rf <- randomForest(sen ~ ., data = mk_off_clim_monthly, ntree = 500, localImp = TRUE)
plot(ice_on_monthly_rf)
...
#Annual
min_depth_frame_off_annual <- min_depth_distribution(ice_off_annual_rf)

#Seasonal
min_depth_frame_off_seasonal <- min_depth_distribution(ice_off_seasonal_rf)

#Monthly
min_depth_frame_off_monthly <- min_depth_distribution(ice_off_monthly_rf)
#head(min_depth_frame_on_annual, n = 10)


#Plot the dist. of min. depth for ice on
#Annual
plot_min_depth_distribution(min_depth_frame_off_annual, min_no_of_trees = 30,  mean_sample = "top_trees")

#Seasonal
plot_min_depth_distribution(min_depth_frame_off_seasonal, min_no_of_trees = 30,  mean_sample = "relevant_trees")

#Monthly
plot_min_depth_distribution(min_depth_frame_off_monthly, min_no_of_trees = 30,  mean_sample = "relevant_trees")
...
#Plot the directions of the interactions
plot_predict_interaction(ice_off_seasonal_rf, mk_off_clim_seasonal, "cmd_sm", "cmd_sp")
