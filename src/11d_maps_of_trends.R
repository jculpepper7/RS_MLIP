...
mk_sen_test <- read_csv(here('data/mk_results.csv'))

#Load HydroLAKES data
hydro_lakes <- read_csv(here('data/hydroLAKES_N_America.csv')) %>% 
  clean_names() %>%
...
rs_results_tbl_ice_on <- read_csv(here('data/rs_results_tbl_ice_on.csv')) %>% 
  mutate(
    Hylak_id = as.factor(Hylak_id),
...
rs_results_tbl_ice_off <- read_csv(here('data/rs_results_tbl_ice_off.csv')) %>% 
  mutate(
    Hylak_id = as.factor(Hylak_id),
    .pred_class = as.factor(.pred_class)
  ) %>% 
...
# 10. Bind Suitable dataframes --------------------------------------------

mk_suitable_data <- ice_on_trend_plotting_data %>% 
  bind_rows(ice_off_trend_plotting_data)

write_csv(mk_suitable_data, here('data/mk_suitable_data.csv'))
